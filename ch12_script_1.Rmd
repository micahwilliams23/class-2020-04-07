...
```{r}
x %>%
  summarize(cor_facultysalary = cor(faculty, earnings),
            cor_avgSAT = cor(sat, earnings))
...
x %>%
  ...
  geom_point() +
  labs(title = 'Student Earnings vs. Faculty Salary')

x %>%
  ggplot(aes(sat, earnings)) +
  geom_smooth(method = lm, se = F) +
  geom_point() +
  labs(title = 'Student Earnings vs. Average SAT Score')
```
...
```{r}
earnings_model <- lm(earnings ~ faculty + sat, data = x)

tidy(earnings_model, conf.int = T) %>%
  select(-c(statistic, p.value))
```

Coefficients: 
- The estimated earning for a school with average faculty salary = 0 and average sat = 0 is $-2590.98$.

faculty:
- A one point increase in average faculty salary increases average earnings by $\$0.3854$

sat:
- A one point increase in average SAT score increases average earnings by $\$24.4804$
...
units: universities
treatment: average SAT score
outcomes: median income
...
```{r}
earning_preds <- predict(earnings_model, 
                         newdata = data.frame(faculty = 50000, sat = c(1400, 1300)))

tibble(avg_1400 = earning_preds[1], 
       avg_1300 = earning_preds[2], 
       effect = avg_1400-avg_1300)
```
...
earnings_model %>% 
  augment() %>%
  filter(.resid %in% c(max(.resid), min(.resid))) %>%
  left_join(x, by = c('faculty', 'sat', 'earnings')) %>%
  select(earnings:.resid, name:tier_name)
```
